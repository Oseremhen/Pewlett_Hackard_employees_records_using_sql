...
--(As managers ID from titles table is 'm0001')
SELECT dept.dept_no AS "department number", dept.dept_name as "department name",
	   emp.emp_no AS "employee number", emp.last_name AS "last name",
...
SELECT dept_emp.dept_no AS "department number", emp.emp_no AS "employee number",
...
SELECT emp.emp_no AS "employee number", emp.last_name AS "last name",
       emp.first_name AS "first name", d.dept_name AS "department name"
FROM employees AS emp
	JOIN dept_emp
	ON (emp.emp_no = dept_emp.emp_no)
		JOIN departments AS d
